...
FROM sonatype/nexus3:3.60.0 as src-plugin
...
#RUN apk add openjdk8
#RUN apk add git
COPY --from=builder --chown=nexus:nexus /build/nexus-datastore-*.jar system/org/sonatype/nexus/nexus-datastore/3.60.0-02/
COPY --from=builder --chown=nexus:nexus /build/nexus-common-*.jar system/org/sonatype/nexus/nexus-common/3.60.0-02/
